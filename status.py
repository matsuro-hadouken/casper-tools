...
def system_memory():
    ...
    sysmemory.addstr(
        "{:.2f} GB".format(float(meminfo["MemTotal"].value) / 1024 / 1024),
        curses.color_pair(4),
    )
    sysmemory.addstr(2, 2, "Mem Avail  : ", curses.color_pair(1))
    sysmemory.addstr(
        "{:.2f} GB".format(float(meminfo["MemAvailable"].value) / 1024 / 1024),
        curses.color_pair(4),
    )
    ...
    sysmemory.addstr(
        3,
        13,
        "{:.2f} %".format(mem_percent),
        curses.color_pair(7 + int(mem_percent / 25)),
    )
...
def system_disk():
    ...
    sysdisk.addstr(
        3,
        13,
        "{:.2f} %".format(disk_percent),
        curses.color_pair(7 + int(disk_percent / 25)),
    )
...
def system_cpu():
    ...
    for key in cpu_checks:
        ...
        interval = "{}{}".format(
            d if d > 0 else h if h > 0 else m if m > 0 else key,
            "d" if d > 0 else "h" if h > 0 else "m" if m > 0 else "s",
        )
        ...
        syscpu.addstr(
            index, 13, "{:.2f}%".format(usage), curses.color_pair(7 + int(usage / 25))
        )
...
def casper_transfers():
    ...
    transfers_view.addstr(
        1,
        2,
        "  Block  / From uref  /  To uref   /         Amount",
        curses.color_pair(4),
    )
    ...
    if length < 1:
        ...
        for key in list(sorted(local_events.keys(), reverse=True)):
            if index <= max_display:
                ...
                transfers_view.addstr(
                    "{}..{}".format(source[:4], source[-4:]),
                    curses.color_pair(1 if source != my_uref else 5),
                )
                transfers_view.addstr(" / ", curses.color_pair(4))
                transfers_view.addstr(
                    "{}..{}".format(target[:4], target[-4:]),
                    curses.color_pair(1 if target != my_uref else 5),
                )
                ...
                transfers_view.addstr(
                    transfer_string.rjust(20, " "), curses.color_pair(5)
                )
...
def casper_deploys():
    ...
    box_height, box_width = peers.getmaxyx()
    deploy_view = curses.newwin(
        2 + (1 if length < 1 else length), 214, 34 + box_height, 0
    )
    deploy_view.box()
        ...
        for key in list(sorted(deploy_dict.keys(), reverse=True)):
            if index < length:
                ...
                deploy_view.addstr(
                    1 + index,
                    2,
                    "{}".format(str(deploy[0]).rjust(8, " ")),
                    curses.color_pair(2 if result == "Failure" else 4),
                )
                ...
                deploy_view.addstr(
                    "{}".format(string.rjust(14, " ")[:14]),
                    curses.color_pair(highlight_color),
                )

                if name:
                    deploy_view.addstr(" / ", curses.color_pair(4))
                    deploy_view.addstr(
                        "{}: ".format("name".rjust(12, " ")),
                        curses.color_pair(base_color),
                    )
                    if name == "caspersign_contract":
                        name = "cs_sgn_cntr"
                    deploy_view.addstr(
                        "{}".format(name.ljust(11, " ")[:11]),
                        curses.color_pair(highlight_color),
                    )
                ...
                if entry:
                    deploy_view.addstr(" / ", curses.color_pair(4))
                    deploy_view.addstr(
                        "{}: ".format("entry".rjust(12, " ")),
                        curses.color_pair(base_color),
                    )
                    if entry == "store_signature":
                        entry = "store_sig"
                    deploy_view.addstr(
                        "{}".format(entry.ljust(11, " ")[:11]),
                        curses.color_pair(highlight_color),
                    )
                ...
                for param in params:
                    ...
                    if param == "amount":
                        ...
                        if param_index > 4:
                            ...
                            deploy_view.addstr(
                                "{}: ".format(
                                    param.rjust(param_area_size, " ")[:param_area_size]
                                ),
                                curses.color_pair(base_color),
                            )
                ...
                if len(params) < 7:
                    ...
                    if not error_message:
                        ...
                        deploy_view.addstr(
                            " / paid: {} {}".format(
                                "{:,.2f}".format(paid_cost / 1000000000)[:4],
                                error_message[:31],
                            ),
                            curses.color_pair(base_color),
                        )
...
def casper_bonds():
    ...
    try:
        ...
        total_stake_str = "{:,} CSPR".format(
            int((staked + delegate_stake) / 1000000000)
        )
        ...
        bonds.addstr(
            "{}".format(our_stake_str.rjust(longest_len, " ")), curses.color_pair(4)
        )
        bonds.addstr(
            " {}".format(millify(int(staked / 1000000000) * float(current_price))),
            curses.color_pair(1),
        )

        bonds.addstr(5, 2, "Delegate : ", curses.color_pair(1))
        bonds.addstr(
            "{}".format(delegate_str.rjust(longest_len, " ")), curses.color_pair(4)
        )
        bonds.addstr(
            " {}".format(
                millify(int(delegate_stake / 1000000000) * float(current_price))
            ),
            curses.color_pair(1),
        )
        ...
        bonds.addstr(
            "{}".format(total_stake_str.rjust(longest_len, " ")), curses.color_pair(4)
        )
        bonds.addstr(
            " {}".format(
                millify(
                    int((staked + delegate_stake) / 1000000000) * float(current_price)
                )
            ),
            curses.color_pair(1),
        )
        ...
        if longest_len > 13:
            bonds.addstr(
                "{} {:d}%".format(
                    our_reward_str.rjust(longest_len, " "), int(reward_percent * 100)
                ),
                curses.color_pair(4),
            )
        else:
            bonds.addstr(
                "{} ({:.2%})".format(
                    our_reward_str.rjust(longest_len, " "), reward_percent
                ),
                curses.color_pair(4),
            )
        ...
        reward_percent = (
            last_del_reward / (delegate_stake if delegate_stake else 1) * 12 * 365
        )
        if longest_len > 13:
            bonds.addstr(
                "{} {:d}%".format(
                    del_reward_str.rjust(longest_len, " "), int(reward_percent * 100)
                ),
                curses.color_pair(4),
            )
        else:
            bonds.addstr(
                "{} ({:.2%})".format(
                    del_reward_str.rjust(longest_len, " "), reward_percent
                ),
                curses.color_pair(4),
            )
        ...
        while currentProposerBlock > lastBlock and self._running:
            try:
                ...
                if transfers:
                    ...
                    root_hash = block_info["result"]["block"]["header"][
                        "state_root_hash"
                    ]
                    for transfer in transfers:
                        ...
                        transfer_dict[
                            "{}-{}-{}-{}".format(
                                str(currentProposerBlock).rjust(8, " "),
                                block_hash,
                                source,
                                target,
                            )
                        ] = [currentProposerBlock, amount, source, target]
    ...
    def run(self):
        ...
        while self._running:
            ...
            while status_not_responding:
                try:
                    peers_info = json.loads(
                        os.popen("curl -s {}:8888/status".format(localhost)).read()
                    )
...
class CoinList(object):
    def __init__(
        self, access_key, access_secret, endpoint_url="https://trade-api.coinlist.co"
    ):
    ...
    def request(self, method, path, params={}, body={}):
        ...
        message = (
            timestamp + method + path_with_params + ("" if not body else json_body)
        )
...
class CoinListTask:
    ...
    def run(self):
        ...
        coinlist = CoinList(
            "50883453-345b-4b11-ade9-105ca81c53fd",
            "YTxYSY7lXzp7Uq26dXnnPeYSQ2g3JYG1nVP/hmJ9u5eGBS/XXf6OjnnnLr5Nr87GW1upSkVcLDDxxX5hnwaAGA==",
        )
                ...
                if "Delegator" in info:
                    ...
                    if val == public_key:
                        ...
                        if my_val_reward > 1000000000:
                            global_events["Last Reward"] = "{:,.4f} CSPR".format(
                                my_val_reward / 1000000000
                            )
                        else:
                            global_events["Last Reward"] = "{:,} mote".format(
                                int(my_val_reward)
                            )

            if my_val_reward > 1000000000:
                global_events["Our Last Reward"] = "{:,.4f} CSPR".format(
                    my_val_reward / 1000000000
                )
            else:
                global_events["Our Last Reward"] = "{:,} mote".format(
                    int(my_val_reward)
                )

            if my_del_reward > 1000000000:
                global_events["Del Last Reward"] = "{:,.4f} CSPR".format(
                    my_del_reward / 1000000000
                )
            else:
                global_events["Del Last Reward"] = "{:,} mote".format(
                    int(my_del_reward)
                )
        ...
        for deploy in deploys:
            ...
            if session:
                for key in session:
                    ...
                    entry = (
                        None
                        if "entry_point" not in session[key]
                        else session[key]["entry_point"]
                    )
                    ...
                    if args:
                        ...
                        deploy_dict[
                            "{}-{}".format(str(height).rjust(8, " "), deploy)
                        ] = [
                            height,
                            key,
                            params,
                    ...
                    if line.startswith("data:"):
                        try:
                            ...
                            if key == "Step":
                                ...
                                try:
                                    ProcessStep(
                                        json_str[key]["execution_effect"]["transforms"],
                                        last_height,
                                    )
                                except:
                            ...
                            if key == "BlockAdded":
                                ...
                                event_time = datetime.strptime(
                                    json_str[key]["block"]["header"]["timestamp"],
                                    "%Y-%m-%dT%H:%M:%S.%fZ",
                                )
                                last_height = int(
                                    json_str[key]["block"]["header"]["height"]
                                )
                                ...
                                if not StepEvents:
                                    try:
                                        era_end = json_str[key]["block"]["header"][
                                            "era_end"
                                        ]
                                        if era_end:
                                            reward = era_end["era_report"]["rewards"][
                                                "{}".format(public_key)
                                            ]
                                            if reward > 1000000000:
                                                ...
                                                global_events[
                                                    "Last Reward"
                                                ] = "{:,} mote".format(int(reward))
                                    except:
                                        global_events["Last Reward"] = "Not Found"

                                try:
                                    proposer = json_str[key]["block"]["body"][
                                        "proposer"
                                    ].strip('"')
                                    if proposer in proposers_dict:
                                        proposers_dict[proposer] = (
                                            proposers_dict[proposer] + 1
                                        )
                                    else:
                                        proposers_dict[proposer] = 1
                                    ...
                                    if proposer == public_key:
                                        era_id = json_str[key]["block"]["header"][
                                            "era_id"
                                        ]
                                        if era_id in our_blocks:
                                            our_blocks[era_id] = our_blocks[era_id] + 1
...
def casper_events():
    ...
    events = curses.newwin(
        2 + 5, events_box_width, events_box_y + events_box_height, events_box_x
    )
    events.box()
    ...
    if length < 1:
        ...
        for key in list(sorted(local_events.keys())):
            if num_events > 5 and key in skip_list:
                ...
                continue
            events.addstr(
                1 + index, 2, "{} : ".format(key.ljust(17, " ")), curses.color_pair(1)
            )
...
def casper_proposers():
    ...
    local_proposers = (
        proposers_dict.copy()
    )  # make a copy in case our thread tries to stomp
    ...
    proposers.addstr(
        " {:6} ".format(
            (global_height - currentProposerBlock)
            if global_height > 0 and currentProposerBlock > 0
            else 0
        ),
        curses.color_pair(5),
    )
    ...
    try:
        if not length:
            ...
            we_are_included = False
            for proposer in sorted(
                local_proposers.items(), key=lambda x: x[1], reverse=True
            ):
                if (
                    index == max_proposers
                    and not we_are_included
                    and proposer[0] != public_key
                ):
                    continue
                ...
                proposers.addstr(
                    index,
                    2,
                    "{}..{} : ".format(proposer[0][:10], proposer[0][-4:]),
                    curses.color_pair(
                        proposer_color if proposer[0] != public_key else 5
                    ),
                )
                proposers.addstr(
                    "{:6.2f}%".format(
                        current_weights[proposer[0]] / total_staked * 100
                    ),
                    curses.color_pair(4),
                )
                proposers.addstr(" / ", curses.color_pair(1))
                proposers.addstr(
                    "{:6.2f}%".format(100 * proposer[1] / blocks), curses.color_pair(4)
                )
            ...
            if not we_are_included and public_key in current_weights:
                proposers.addstr(
                    index,
                    2,
                    "{}..{} : ".format(public_key[:10], public_key[-4:]),
                    curses.color_pair(5),
                )
                if public_key in local_proposers:
                    proposers.addstr(
                        "{:6.2f}%".format(
                            current_weights[public_key] / 3500000000000000000 * 100
                        ),
                        curses.color_pair(4),
                    )
                    proposers.addstr(" / ", curses.color_pair(1))
                    proposers.addstr(
                        "{:6.2f}%".format(100 * local_proposers[public_key] / blocks),
                        curses.color_pair(4),
                    )
                else:
                    proposers.addstr(
...
def casper_era_rewards():
    ...
    era_rewards = curses.newwin(
        2 + (max_print * 2) + 2,
        events_box_width,
        events_box_y + events_box_height,
        events_box_x,
    )
    ...
    for era in sorted(era_rewards_dict.items(), key=lambda x: x[0], reverse=True):
        era_rewards.addstr(
            index, 2, "{} Reward : ".format(era[0]), curses.color_pair(1)
        )
        era_rewards.addstr(
            "{:10,.4f} CSPR".format((era[1] / num_era_rewards[era[0]]) / 1000000000),
            curses.color_pair(4),
        )
        index += 1
        current_print += 1
    ...
    for era in sorted(era_block_start.items(), key=lambda x: x[0], reverse=True):
        ...
        if diff != 0:
            era_rewards.addstr(
                index, 2, "{} Blocks : ".format(era[0]), curses.color_pair(1)
            )
...
def casper_finality():
    ...
    finality = curses.newwin(
        2 + (1 if missing_val < 1 else missing_val), 40, starty, 70
    )
    ...
    if not len(missing_validators):
        finality.addstr(
            index,
            2,
            "Checking Finality Signatures" if length > 0 else "Waiting for next Event",
            curses.color_pair(5),
        )
    else:
        for missing in missing_validators:
            finality.addstr(
                index,
                2,
                "{}....{}".format(missing[:16], missing[-16:]),
                curses.color_pair(1 if missing != public_key else 2 if blink else 20),
            )
            index = index + 1
...
def casper_peers():
    ...
    if peer_scan_running:
        ...
        peers.addstr(
            "Next scan in ~{}m {:02}s".format(minutes, seconds), curses.color_pair(5)
        )
    else:
    ...
    peers.addstr(
        "{}".format(len(trusted_ips) + len(trusted_blocked)), curses.color_pair(4)
    )
    ...
    if peers_total and "localhost" in local_peer_scan_dict:
        ...
        for ip in local_peer_scan_dict:
            ...
            if current_peer == None:
                ...
                if peer_next_upgrade != None:
                    ...
                    if (
                        our_era_upgrade < peer_era_upgrade
                        and our_chain == current_peer[2]
                    ):
        ...
        peers.addstr(
            "{:.2%} ({})".format(peers_blocked / peers_total, peers_blocked),
            curses.color_pair(4),
        )

        peers.addstr(3, 25, "Bad Chain : ", curses.color_pair(1))
        peers.addstr(
            "{:.2%} ({})".format(peers_wrong_chain / peers_total, peers_wrong_chain),
            curses.color_pair(4),
        )
        peers.addstr(3, 48, "Bad Ver : ", curses.color_pair(1))
        peers.addstr(
            "{:.2%} ({})".format(
                peers_wrong_version / peers_total, peers_wrong_version
            ),
            curses.color_pair(4),
        )
        ...
        if we_have_not_staged:
            peers.addstr(
                4,
                2,
                "It appears someone has an upgrade staged for",
                curses.color_pair(5),
            )
            ...
            peers.addstr(
                "{:.2%}".format(peers_staked / total_staked), curses.color_pair(4)
            )
            ...
            if our_next_upgrade == None:
                ...
                peers.addstr(
                    "{:.2%} ({})".format(
                        (peers_total - peers_blocked - peers_not_upgraded)
                        / peers_total,
                        peers_total - peers_blocked,
                    ),
                    curses.color_pair(4),
                )
            else:
                ...
                peers.addstr(
                    "{:.2%} ({})".format(
                        peers_not_upgraded / peers_total, peers_not_upgraded
                    ),
                    curses.color_pair(4),
                )
...
def casper_launcher():
    ...
    key_value = (
        os.popen(
            "systemctl show casper-node-launcher.service | grep -e MemoryCurrent -e ActiveState -e LoadState -e FragmentPath -e StateChangeTimestamp="
        )
        .read()
        .split("\n")
    )
    ...
    if active == "active":
        ...
        launcher.addstr(
            "{:.2f} GB".format(float(memory) / 1024 / 1024 / 1024), curses.color_pair(4)
        )
    ...
    launcher.addstr(
        "{} UTC".format(dt.strftime("%Y-%m-%d %H:%M:%S")), curses.color_pair(5)
    )
...
def casper_block_info():
    ...
    try:
        ...
        try:
            ...
            last_block_time = datetime.strptime(
                last_added_block_info["timestamp"], "%Y-%m-%dT%H:%M:%S.%fZ"
            )
        except:
            global_height = 0
            local_height = "null"
    ...
    if peer_address:
        try:
            try:
                peer_status = json.loads(
                    os.popen("curl -m 2 -s {}:8888/status".format(peer_address)).read()
                )
                ...
                if previous_peer:
                    ...
                    peer_status = json.loads(
                        os.popen(
                            "curl -m 2 -s {}:8888/status".format(peer_address)
                        ).read()
                    )
    ...
    block_info.addstr(
        "{} UTC".format(
            last_block_time.strftime("%Y-%m-%d %H:%M:%S") if last_block_time != 0 else 0
        ),
        curses.color_pair(5),
    )
    ...
    block_info.addstr(
        ...
        34,
        "<- {} Peer : ".format(
            "   From" if peer_address not in trusted_ips else "Trusted"
        ),
        curses.color_pair(1),
    )
    block_info.addstr(
        "{}".format(peer_address),
        curses.color_pair(4 if peer_address not in trusted_ips else 5),
    )
    ...
    round_string = "Elapsed {:01d}m {:02d}s {:03d}ms".format(
    ...
    for each_char in round_string:
        block_info.addstr(
            each_char,
            curses.color_pair(
                7 if string_start_x + char_index < 34 + num_blocks else 3
            ),
        )
        char_index += 1
    ...
    block_info.addstr(
        "{}".format(next_upgrade), curses.color_pair(4 if next_upgrade == None else 5)
    )
    ...
    if local_era in era_block_start and local_era - 1 in era_block_start:
        ...
        block_percent = int(
            (era_block_start[local_era] - era_block_start[local_era - 1])
            / avg_num_blocks
            * 100
        )
    ...
    if next_upgrade:
        ...
        if day:
            round_string = "Era {} in ~{}d {}h {:02d}m {:02d}s".format(
                next_era_upgrade, day, hour, minutes, seconds
            )
        elif hour:
            round_string = "Era {} in ~{}h {:02d}m {:02d}s".format(
                next_era_upgrade, hour, minutes, seconds
            )
        elif minutes:
            round_string = "Era {} in ~{}m {:02d}s".format(
                next_era_upgrade, minutes, seconds
            )
        else:
        ...
        block_info.addstr(
            index, int(string_start_x), round_string, curses.color_pair(18)
        )
    ...
    block_info.addstr(
        "{}...{}".format(current_proposer[:24], current_proposer[-24:]),
        curses.color_pair(4 if current_proposer != public_key else 5),
    )
    ...
    block_info.addstr(
        "{}..{}".format(root_hash[:8], root_hash[-8:]), curses.color_pair(4)
    )
    ...
    try:
        ...
        if transfers:
            ...
            for transfer in transfers:
                ...
                target = transfer["target"].strip('"')
                transfer_dict[
                    "{}-{}-{}-{}".format(currentBlock, block_hash, source, target)
                ] = [currentBlock, amount, source, target]
        ...
        if balance > 100000000:
            pub_key_win.addstr(
                "{:,.4f} CSPR".format(balance / 1000000000), curses.color_pair(4)
            )
        else:
            pub_key_win.addstr("{:,} mote".format(balance), curses.color_pair(4))

        pub_key_win.addstr(
            " ({})".format(millify((balance / 1000000000) * float(current_price))),
            curses.color_pair(1),
        )

        coin_len = len(current_price)
        pub_key_win.addstr(
            2, 70 - coin_len - 8 - 7 - 4, "Price: ", curses.color_pair(1)
        )
...
def casper_validator():
    ...
    validator.addstr(
        "{:,} / {:,} / {:,} / {}".format(
            num_cur_validators, num_fut_validators, len(bid_info), validator_slots
        ),
        curses.color_pair(4),
    )
    validator.addstr(
        1,
        42,
        "<- {}/{}/Bids/Slots".format(current_era, future_era),
        curses.color_pair(1),
    )
    ...
    validator.addstr(
        2, 2, "ERA {} : ".format(str(current_era).ljust(8, " ")), curses.color_pair(1)
    )
    validator.addstr(
        "{}".format(current_str.rjust(longest_len, " ")), curses.color_pair(4)
    )
    validator.addstr(
        2, 42, "<- Position {}".format(current_index), curses.color_pair(1)
    )

    validator.addstr(
        3, 2, "ERA {} : ".format(str(future_era).ljust(8, " ")), curses.color_pair(1)
    )
    validator.addstr(
        "{}".format(future_str.rjust(longest_len, " ")), curses.color_pair(4)
    )
    ...
    reward = (
        our_era_rewards[current_era - 1] if current_era - 1 in our_era_rewards else 0
    )
    if reward > 10000000:
    ...
    validator.addstr(
        "{}".format(this_str.rjust(longest_len, " ")), curses.color_pair(4)
    )
    if current_weight:
        validator.addstr(
            4,
            42,
            "<- {:.2%} APR".format(
                (reward / current_weight) * 12 * 365,
                "s" if len(our_rewards) > 1 else "",
            ),
            curses.color_pair(1),
        )
    ...
    if current_weight:
        validator.addstr(
            5,
            42,
            "<- Last {} reward{} ({:.2%})".format(
                len(our_rewards),
                "s" if len(our_rewards) > 1 else "",
                ((reward / current_weight) if current_weight else 0) * 12 * 365,
            ),
            curses.color_pair(1),
        )
    else:
        validator.addstr(
            ...
            42,
            "<- Last {} reward{}".format(
                len(our_rewards), "s" if len(our_rewards) > 1 else ""
            ),
            curses.color_pair(1),
        )
    ...
    validator.addstr(
        ...
        42,
        "<- {}/{}/{}/Avg".format(
            current_era_global, current_era_global - 1, current_era_global - 2
        ),
        curses.color_pair(1),
    )
...
def main():
    ...
    try:
        config_file = (
            os.popen(
                "ps -ef | grep casper | grep config.toml | head -1 | awk -F\" \" '{print $10}'"
            )
            .read()
            .split("\n")[0]
        )
    ...
    if config_file == "" or config_file == None:
        try:
            subfolders = [
                f.path
                for f in os.scandir("/etc/casper/")
                if f.is_dir() and re.match(r"\d{0,255}_\d{0,255}_\d{0,255}", f.name)
            ]
    ...
    try:
        known = (
            config.get("network", "known_addresses")
            .strip("[]")
            .replace(" ", "")
            .split(",")
        )
