...
#        casper.clear()
        ...
        casper.addstr(index, 0, 'MemoryCurrent: ', curses.color_pair(1))
        casper.addstr('{}'.format(json_dict['MemoryCurrent']), curses.color_pair(4))
        index += 1
        casper.addstr(index, 0, 'ActiveState  : ', curses.color_pair(1))
        casper.addstr('{}'.format(json_dict['ActiveState']), curses.color_pair(4))
        index += 1
        casper.addstr(index, 0, 'LoadState    : ', curses.color_pair(1))
        casper.addstr('{}'.format(json_dict['LoadState']), curses.color_pair(4))
        index += 1
        casper.addstr(index, 0, 'FragmentPath : ', curses.color_pair(1))
        casper.addstr('{}'.format(json_dict['FragmentPath']), curses.color_pair(4))
        index += 1
        casper.addstr(index, 0, 'Timestamp    : ', curses.color_pair(1))
        casper.addstr('{}'.format(json_dict['StateChangeTimestamp']), curses.color_pair(4))
        index += 1


        index += 1
        casper.addstr(index, 0, 'Public Key   : ', curses.color_pair(1))
        casper.addstr('{}'.format(public_key), curses.color_pair(4))
        index += 1

        global_status = json.loads(os.popen('curl -s localhost:8888/status | jq -r .last_added_block_info').read())
        try:
        ...
        index += 1
        casper.addstr(index, 0, 'Local height : ', curses.color_pair(1))
        casper.addstr('{}'.format(local_height), curses.color_pair(4))

        index += 1
        casper.addstr(index, 0, 'Chain height : ', curses.color_pair(1))
        casper.addstr('{}'.format(global_height), curses.color_pair(4))
        index += 1
        casper.addstr(index, 0, 'Round Length : ', curses.color_pair(1))
        casper.addstr('{}'.format(round_length), curses.color_pair(4))
        index += 1
        casper.addstr(index, 0, 'Next Upgrade : ', curses.color_pair(1))
        casper.addstr('{}'.format(next_upgrade), curses.color_pair(4))
        index += 1
        casper.addstr(index, 0, 'Build Version: ', curses.color_pair(1))
        casper.addstr('{}'.format(build_version), curses.color_pair(4))

        index += 2
        casper.addstr(index, 0, 'Chain Name   : ', curses.color_pair(1))
        casper.addstr('{}'.format(chain_name), curses.color_pair(4))
        index += 1
        casper.addstr(index, 0, 'Starting Hash: ', curses.color_pair(1))
        casper.addstr('{}'.format(root_hash), curses.color_pair(4))

        index += 2
        casper.addstr(index, 0, 'API Version  : ', curses.color_pair(1))
        casper.addstr('{}'.format(api_version), curses.color_pair(4))
        index += 1
        casper.addstr(index, 0, 'Local ERA    : ', curses.color_pair(1))
        casper.addstr('{}'.format(local_era), curses.color_pair(4))

        index += 2
        casper.addstr(index, 0, 'ERA {}       : '.format(local_era), curses.color_pair(1))
        casper.addstr('{}'.format(era_current_weight), curses.color_pair(4))
        index += 1
        casper.addstr(index, 0, 'ERA {}       : '.format(local_era+1), curses.color_pair(1))
        casper.addstr('{}'.format(era_future_weight), curses.color_pair(4))
        index += 1
        casper.addstr(index, 0, 'Last Reward  : ', curses.color_pair(1))
        ...
        casper.addstr(height-1, 0, statusbarstr)
